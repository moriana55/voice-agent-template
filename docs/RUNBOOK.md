...
- Persistent data: `DATA_DIR/call-records.jsonl`, `DATA_DIR/usage-events.jsonl`, and TTL-bounded encrypted `DATA_DIR/web-sessions.enc.json` / `DATA_DIR/telephony-sessions.enc.json` when their respective storage modes are enabled
...
4. Set `PUBLIC_BASE_URL`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER`, `TELEPHONY_SESSION_STORAGE=encrypted-file`, and an approved `TELEPHONY_SESSION_TTL_MINUTES` before enabling telephone traffic.
...
The repository implements encrypted application storage, atomic session-file replacement, retention pruning, and a streaming encrypted off-platform export; it does not create infrastructure snapshots. Railway's Backups screen currently reports that backups/PITR require the Pro plan for this Hobby workspace. Before customer mode is enabled, either upgrade and configure daily plus weekly volume backups or approve the off-platform workflow in [`BACKUP_RESTORE.md`](BACKUP_RESTORE.md). A restore drill must create an isolated replacement volume, retain the original, use the matching encryption keys, and pass readiness, record decryption, and restart-resume checks before traffic is switched.
